...
# 导入模型
model = load_model('./model/model_11_15_19_34.h5')
...
while True:      
    ...
    if frame is not None:
        ...
        #检查人脸s
        faces = face.detectMultiScale(frame,1.1,3,0,(100,100))
        # new_img = 
        for(x,y,w,h) in faces:
            i = i + 1
            # tmp_img = cv2.resize(frame[y:y+h,x:x+w],(200,200))
            # tmp_img = cv2.resize(frame[y+int(0.05*h):y+int(h*0.95), x+int(w*0.05):x+int(w*0.95)],(227,227))
            tmp_img = cv2.resize(frame[y:y+h+20, x:x+w],(227,227))
            ...
            cv2.rectangle(frame,(int(x), int(y)),(int(x+w), int(y+h+20)),(0, 0, 255),2)
